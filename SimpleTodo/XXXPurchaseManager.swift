...
class XXXPurchaseManager: NSObject , SKPaymentTransactionObserver {
    ...
    func startWithProduct(product : SKProduct){
        ...
        if SKPaymentQueue.canMakePayments() == false {
            let purchaseInvalid:String = NSLocalizedString("purchaseInvalid", comment: "購入設定無効")
            
            errorCount += 1
            errorMessage = purchaseInvalid
        }
        
        if self.productIdentifier != nil {
            let purchaseProcessing:String = NSLocalizedString("purchaseProcessing", comment: "課金処理中")
            
            errorCount += 10
            errorMessage = purchaseProcessing
        }
        
        if self.isRestore == true {
            let restoreProcessing:String = NSLocalizedString("restoreProcessing", comment: "リストア処理中")
            
            errorCount += 100
            errorMessage = restoreProcessing
        }
            ...
            for transaction in transactions {
                ...
                if transaction.payment.productIdentifier == product.productIdentifier {
                    if let window = UIApplication.sharedApplication().delegate?.window {
                        let purchaseInterruption:String = NSLocalizedString("purchaseInterruption", comment: "購入中断")
                        let continueTitle:String = NSLocalizedString("continue", comment: "続行")
                        
                        let ac = UIAlertController(title: nil, message: "\(product.localizedTitle)" + purchaseInterruption, preferredStyle: .Alert)
                        let action = UIAlertAction(title: continueTitle, style: UIAlertActionStyle.Default, handler: {[weak self] (action : UIAlertAction!) -> Void in
    ...
    func startRestore(){
        if self.isRestore == false {
            ...
            let error = NSError(domain: "PurchaseErrorDomain", code: 0, userInfo: [NSLocalizedDescriptionKey:restoreProcessing])
            self.delegate?.purchaseManager?(self, didFailWithError: error)
        }
    }
